...
const Roadmap = ({ statuses }) => {
    return <div className={`roadmap bg-white rounded-xl p-6 flex flex-col gap-6`}>
        <div className="flex justify-between items-center">
            ...
            <Link to="/roadmap" className="text-secondary-purple font-semibold text-sm default-link">View</Link>
        </div>
        <ul className="flex flex-col gap-2">
            {statuses.map(status => {
                return <li key={uuid()} className="flex justify-between items-center">
